...
namespace modeling_of_solids
{
	public partial class AtomicModel
	{
		...
		private Vector Periodic(Vector pos)
		{
			...
			if (pos.X > BoxSize) newPos.X = pos.X - BoxSize;
			else if (pos.X < 0) newPos.X = BoxSize + pos.X;
			else newPos.X = pos.X;

			if (pos.Y > BoxSize) newPos.Y = pos.Y - BoxSize;
			else if (pos.Y < 0) newPos.Y = BoxSize + pos.Y;
			else newPos.Y = pos.Y;

			if (pos.Z > BoxSize) newPos.Z = pos.Z - BoxSize;
			else if (pos.Z < 0) newPos.Z = BoxSize + pos.Z;
			else newPos.Z = pos.Z;
		...
		public PointD[] GetRadialDistribution()
		{
			...
			foreach (var atom in Atoms)
				if (atom.Position.X > 0.25 * BoxSize && atom.Position.X < 0.75 * BoxSize &&
